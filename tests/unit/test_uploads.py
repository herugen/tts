...
class TestUploadEndpoint:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
            ...
            with open(temp_file.name, "rb") as f:
                response = test_client.post(
                    "/api/v1/uploads", files={"file": ("test.wav", f, "audio/wav")}
                )
    ...
    def test_upload_unsupported_file_type(self, test_client):
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as temp_file:
            ...
            with open(temp_file.name, "rb") as f:
                response = test_client.post(
                    "/api/v1/uploads", files={"file": ("test.txt", f, "text/plain")}
                )
    ...
    def test_upload_file_too_large(self, test_client):
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
            ...
            with open(temp_file.name, "rb") as f:
                response = test_client.post(
                    "/api/v1/uploads", files={"file": ("large.wav", f, "audio/wav")}
                )
    ...
    def test_upload_mp3_file(self, test_client):
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as temp_file:
            ...
            with open(temp_file.name, "rb") as f:
                response = test_client.post(
                    "/api/v1/uploads", files={"file": ("test.mp3", f, "audio/mpeg")}
                )
